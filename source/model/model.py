...
from model_utils import BaseNet, NetworkOutput, Action, HiddenState
...
class DynamicsNetwork(nn.Module):
    def __init__(
        ...
        self.fc_reward = mlp(num_agents * (hidden_state_size + action_space_size),
                             fc_reward_layers, full_support_size, use_value_out=True)

    def forward(self, hidden_state: torch.Tensor, action: torch.Tensor):
        # shape of hidden_state: (batch_size, num_agents, hidden_state_size)
        # shape of action: (batch_size, num_agents, action_space_size)
        batch_size, num_agents = hidden_state.size(0), hidden_state.size(1)
        pre_state = hidden_state

        attn_input = torch.cat([hidden_state, action], dim=2)
        attn = self.attention_stack(attn_input)

        concat_input = torch.cat([hidden_state, action, attn], dim=2)
        state = self.fc_dynamic(concat_input.reshape(batch_size * num_agents, -1)).reshape(batch_size, num_agents, -1)
        state += pre_state

        reward = self.fc_reward(torch.cat([state, action], dim=2).reshape(batch_size, -1))
...
class MAMuZeroNet(BaseNet):
    ...
    def dynamics(self, hidden_state: torch.Tensor, action: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        next_hidden_state, reward = self.dynamics_network(hidden_state.reshape(batch_size, self.num_agents, self.hidden_state_size),
                                                          action_onehot.reshape(batch_size, self.num_agents, self.action_space_size))
        next_hidden_state = next_hidden_state.reshape(batch_size, self.num_agents * self.hidden_state_size)
