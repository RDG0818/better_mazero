...
from model.model_utils import BaseNet
from model.model import MAMuZeroNet 
from smac.smac_wrapper import SMACWrapper 
...
class SMACGameConfig(BaseConfig):
    def __init__(self, args: argparse.Namespace):
        super().__init__(args)

        if self.use_vectorization:
            self.reward_support = DiscreteSupport(min_val=-5.0, max_val=5.0)
            self.value_support = DiscreteSupport(min_val=-5.0, max_val=5.0)

        self.model_observation_shape_for_net_init = (self.model_input_observation_dim,)

    def new_game(self) -> SMACWrapper:
        return SMACWrapper(map_name=self.env_name, seed=self.seed)

    def get_uniform_network(self) -> MAMuZeroNet:
        return MAMuZeroNet(
            num_agents=self.num_agents,
            observation_shape=self.model_observation_shape_for_net_init,
            action_space_size=self.action_space_size,
            hidden_state_size=self.hidden_state_size,
            fc_representation_layers=self.fc_representation_layers,
            ...
            use_feature_norm=self.use_feature_norm,
            # MAZero specific, if your MAMuZeroNet uses them:
